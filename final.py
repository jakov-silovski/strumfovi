...
def poligon1():      #funkcija za crtanje 1. poligona
    ...
    ymax=650   #
    coord=list()
    pozadina=pygame.image.load('{}\slika1.png'.format(path))
...
def poligon2():      #funkcija za crtanje 2. poligona
    ...
    ymax=650
    coord=list()
    pozadina=pygame.image.load('{}\slika2.png'.format(path))
    pygame.draw.polygon(surface=screen,color='black',points=[(xmin,ymin),(xmax,ymin),(xmax,ymax),(xmin,ymax)],width=deb)
...
def poligon3():
    ...
    ymax=650
    coord=list()
    pozadina=pygame.image.load('{}\slika3.png'.format(path))
    pygame.draw.polygon(surface=screen,color='black',points=[(xmin,ymin),(xmax,ymin),(xmax,ymax),(xmin,ymax)],width=deb)
...
def lovac_grafika(t1,t2): #ovdje su definirani skinovi za lovca i crtanje lovca
    ...
    if nivo==1:
        lovac1= pygame.image.load('{}\lovac113.png'.format(path))#skin za lovca
        screen.blit(lovac1,(t1,t2))                                                     #     
    if nivo==2:
        lovac2=pygame.image.load('{}\lovac114.png'.format(path))
        screen.blit(lovac2,(t1,t2))
    if nivo==3:
        lovac3=pygame.image.load('{}\lovac115.png'.format(path))
...
def lik_grafika(p1,p2): #funkcija definira crtanje lika i njegovog skina
    ...
    #crtanje skina
    igr=pygame.image.load('{}\igrac.png'.format(path))
...
def hrana_grafika(x1,x2): #u ovoje se funkciji definira grafika hrane 
    ...
    hranaslika= pygame.image.load('{}\hrana.png'.format(path))#skin za hranu
    screen.blit(hranaslika,(x1,x2))
...
# buduci da se igrica prebacuje na razlicita racunala koristim apsolutnu putanju
    ...
    if game_state == "start_menu":
        ...
        if keys[pygame.K_s]:
            ...
            if nivo == 1:
                ...
                T1=t1;T2=t2
                T3=t3;T4=t4
                mixer.music.load('{}\drugi.mp3'.format(path))
                mixer.music.play(-1)
                tic=time.time()
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN: 
                ...
                #ako je pritisnuto enter znaci da je gotovo upisivanje
                    ...
                    game_state='game'
                    mixer.music.load('{}\prvi.mp3'.format(path)) #za stavljanje muzike
                    mixer.music.play(-1)
                    tic=time.time()
        ...
        for i in range(len(eat)):
            ...
            if collide:
                ...
                eat_sound = mixer.Sound('{}\eat.mp3'.format(path))
        ...
        if len(eat)==0:
            ...
            if nivo == 1:
                ...
                T1=t1;T2=t2
                T3=t3;T4=t4
                mixer.music.load('{}\prvi.mp3'.format(path))
                mixer.music.play(-1)
            elif nivo == 2:
                xmin,xmax,ymin,ymax,coords,eat,pozadina=poligon2()
                p1,p2,step,deb,t1,t2,t3,t4,br=pozicije1()
